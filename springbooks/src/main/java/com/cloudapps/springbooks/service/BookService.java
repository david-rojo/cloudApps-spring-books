...
public class BookService {
	...
	@Autowired
	public BookService(@Value("${springbooks.config.startup.dataload:false}") boolean startupDataLoad) {
		if (startupDataLoad) {
			this.save(new Book(
					"Don Quixote", 
			...
			this.save(new Book(
					...
					"CloudApps Classics",
					1943));	
			log.info("Startup book load completed");
		}
		else {
			log.info("Startup book load disabled");
		}
